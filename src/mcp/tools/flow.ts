...
const startFlowTool: ToolDefinition = {
  ...
  title: 'Show the menu',
  description:
    'Returns the menu of the two things this server does, plus a session_id. CALL THIS FIRST ' +
    ...
    'want another. Print the two options exactly as returned and wait. Add no preamble, no ' +
    'explanation of the options, no recommendation and no question of your own, and begin no ' +
    'work until the user picks one. The options are: 1 generate storyboard, 2 read handbook ' +
    'content. Answer with flow_choose. Sessions are independent and ' +
...
const flowChooseTool: ToolDefinition = {
  ...
  description:
    ...
    'done: it asks nothing more and generation starts. The reading flow runs menu -> subject ' +
    '-> module -> unit. In the reading flow an answer that matches no option is tried as a ' +
    'topic name before it is rejected, so ' +
